'use client';
import { ActionBar, Box, CloseButton, Flex, Span, Table } from '@chakra-ui/react';
...
export const DataTableContainer: FC<TableProps> = ({
  ...
  if (data?.length === 0) {
    return <NoDataAnimation animationType={animationType} notFoundTitle={notFoundTitle} />;
  }
                  ...
                  {col.accessor === 'select' ? (
                    ...
                    <>
                      ...
                      {sortConfig?.key === col.accessor &&
                        ...
                        (data?.length > 1 ? (sortConfig.direction === 'asc' ? '⬆' : '⬇') : null)}
                    </>
                  )}
                </Table.ColumnHeader>
              ))}
                  ...
                  <Table.Cell p={'15px'} key={rowIndex}>
                    {col.accessor === 'select' ? (
                      ...
                      <Flex justifyContent={'center'} width={'fit-content'}>
                        <DataTableActionButtons key={item.id} actions={col?.actions} item={item} />
                      </Flex>
                    ) : col.cell ? (
                      col.cell(col.accessor === 'fullObject' ? item : item[col.accessor])
                    ) : (
                      item[col.accessor]
                    )}
                  </Table.Cell>
