...
export const BaseRadioCard: FC<IRadioCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <HStack width={'full'} alignItems={'center'} flexDir={{ base: 'column', md: 'row' }}>
        <For each={items}>
          {(item, index) => (
            <RadioCard.Item key={index} width={'full'} value={item?.value}>
              <RadioCard.ItemHiddenInput />
              <Flex gap={2}>
                <RadioCard.ItemControl alignItems={'center'} justifyItems={'center'}>
                  <BaseIcon
                    color={
                      items[currentIndex]?.value === item.value ? 'primary.500' : 'secondary.500'
                    }
                  >
                    {item.icon}
                  </BaseIcon>
      ...
      {items[currentIndex]?.desc && (
        <RadioCard.ItemDescription mt={5}>{items[currentIndex]?.desc}</RadioCard.ItemDescription>
      )}
      <For each={items}>
        {(item, index) => (
          <Box
