  ...
  return (
    ...
    >
      <Box display={'flex'} justifyContent={'center'} alignItems={'center'} width="100%">
        <Flex
          width={'full'}
          bg={isActiveLink(subLink.path!) ? hexToRGB('neutral', 0.3) : 'transparent'}
          me={'10px'}
        ...
        >
          {subLink?.icon ? (
            ...
            <GoDotFill width="9px" height="9px" color={setMenuItemPointStyle(subLink.path)} />
          )}
          <BaseText {...setMenuItemTextStyle(subLink.path)}>{t(subLink?.label)}</BaseText>
        </Flex>
      </Box>
    </Link>
  );
