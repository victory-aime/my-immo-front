...
export const Links: FC<SubMenuProps> = ({ sideToggled, redirectToPath, isActiveLink, link }) => {
    ...
    >
      ...
      >
        {link.icon && (
          <BaseIcon
            width={'45px'}
            color={isActive ? VariablesColors.primary : VariablesColors.transparent}
          >
            <link.icon fill={isActive ? VariablesColors.white : VariablesColors.grayScale} />
          </BaseIcon>
        )}

        <Box
          display={!sideToggled ? { lg: 'none' } : { base: 'block', lg: 'block' }}
          width={'full'}
          ms="0.5rem"
        >
