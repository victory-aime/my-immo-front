...
const agencyContactListQueries = (args: QUERIES.QueryPayload<MODELS.IAgencyCommonParams>) => {
  const { params } = args;
  return QUERIES.useCustomQuery<MODELS.IAgencyRequestList[]>({
    queryKey: [Constants.CONTACT_KEYS.AGENCY_CONTACT_LIST],
    queryFn: () => contactServiceInstance().agencyContactList(params as MODELS.IAgencyCommonParams),
...
const changeAgencyContactStatusMutation = (
  ...
  return QUERIES.useCustomMutation({
    ...
    mutationFn: ({ params }) =>
      contactServiceInstance().agencyChangeContactStatus(params! as MODELS.IAgencyCommonParams),
...
const readAllAgencyContactMutation = (
  ...
  return QUERIES.useCustomMutation({
    ...
    mutationFn: ({ params }) => contactServiceInstance().readAllAgencyContact(params!),
    options: args.mutationOptions,
  });
};

const publicContactRequestMutation = (args: QUERIES.MutationPayload<MODELS.IContact>) => {
  return QUERIES.useCustomMutation({
    mutationKey: [Constants.CONTACT_KEYS.PUBLIC_CONTACT_REQUEST],
    mutationFn: ({ payload }) => contactServiceInstance().publicContact(payload!),
    options: args.mutationOptions,
  });
};
