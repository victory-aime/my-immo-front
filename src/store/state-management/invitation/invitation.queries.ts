...
const createInvitationMutation = (
  args: QUERIES.MutationPayload<MODELS.ICreateInvitation>,
) => {
  return QUERIES.useCustomMutation({
    mutationKey: [Constants.INVITE_KEYS.CREATE_INVITATION],
    mutationFn: ({ payload }) =>
      invitationServiceInstance().createInvitation(payload!),
    options: args.mutationOptions,
  });
};

const acceptInvitationMutation = (
  args: QUERIES.MutationPayload<{ token: string }>,
) => {
  return QUERIES.useCustomMutation({
    mutationKey: [Constants.INVITE_KEYS.ACCEPT_INVITATION],
    mutationFn: ({ params }) =>
      invitationServiceInstance().acceptInvitation(params!.token),
    options: args.mutationOptions,
  });
};

const cancelInvitationMutation = (
  args: QUERIES.MutationPayload<{ inviteId: string }>,
) => {
  return QUERIES.useCustomMutation({
    mutationKey: [Constants.INVITE_KEYS.CANCEL_INVITATION],
    mutationFn: ({ params }) =>
      invitationServiceInstance().cancelInvitation(params!.inviteId),
    options: args.mutationOptions,
  });
};
